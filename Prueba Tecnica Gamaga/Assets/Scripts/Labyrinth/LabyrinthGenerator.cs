...
public class LabyrinthGenerator : MonoBehaviour
{
    ...
    Labyrinth labyrinthToReturn;

    public GameObject labyrinthFloor;

    Vector3 centerOfLabyrinth;

    public void InitializeLabyrinthGenerator(int size)
    {
        prevSize = LabyrinthSize;
        ...
        if(availableBlocksContainer != null)
            Destroy(availableBlocksContainer);
        availableBlocksContainer = new GameObject("Available Blocks");

        if (usedBlocksContainer != null)
            Destroy(usedBlocksContainer);
        ...
        int blockAmount = Mathf.CeilToInt(size * size * initialBlocksFactor / allBlockTypes.Count);
    ...
    public Labyrinth GenerateLabyrinth(int size)
    {
        ...
        LabyrinthSize = size;
        if (!isInitialized || LabyrinthSize != prevSize)
        ...
        labyrinthToReturn = new Labyrinth(blocksMatrix, entranceBlock, exitBlock); //creates new labyrinth with data that is already set, so we can store it's solution on the next step

        FixLabyrinthExit(exitBlock);

        centerOfLabyrinth = new Vector3(((float)LabyrinthSize - 1) * 3 / 2, 1, ((float)LabyrinthSize - 1) * 3 / 2);

        //Adjust camera to fit labyrinth on screen
        Camera.main.transform.position = centerOfLabyrinth;
        Camera.main.orthographicSize = LabyrinthSize*1.7f;

        SetLabyrinthFloor();

        return labyrinthToReturn;
    }
    ...
    {
        Node solution = PathFindingTool.FindThePath(blocksMatrix, entranceBlock, exitBlock);
        if (solution == null)
        {
            if (exitBlock == null)
                return;
            ...
            solution = PathFindingTool.FindThePath(blocksMatrix, entranceBlock, exitBlock);
            if (solution == null)
            {
        ...
        labyrinthToReturn.solution = solution;
    }

    public void SetLabyrinthFloor()
    {
        if (labyrinthFloor == null)
        {
            labyrinthFloor = GameObject.Find("LabyrinthFloor");
            if (labyrinthFloor == null)
            {
                labyrinthFloor = GameObject.CreatePrimitive(PrimitiveType.Plane);
                labyrinthFloor.name = "LabyrinthFloor";
            }
        }
        if (labyrinthFloor == null)
            Debug.LogError("There must be a floor!");

        labyrinthFloor.transform.localScale = new Vector3(LabyrinthSize / 3f, 1, LabyrinthSize / 3f);
        labyrinthFloor.transform.position = new Vector3(centerOfLabyrinth.x, -0.5f, centerOfLabyrinth.z);
    }
